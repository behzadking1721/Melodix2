...
import { Play, FolderOpen, CheckCircle, FileText, LayoutGrid, List } from 'lucide-react';
  ...
  if (isPlaylistView) {
    return (
      <div className="p-10 pb-32 animate-in fade-in duration-500">
        <div className="mb-12">
          <h2 className="text-5xl font-black mb-1 tracking-tighter text-white">Playlist</h2>
          <p className="text-zinc-500 font-black uppercase text-[9px] tracking-[0.3em]">AI-Generated Flows</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
          {playlists.map(p => (
            <div key={p.id} onClick={() => onPlaylistSelect?.(p.id)} className="group cursor-pointer">
              <div className="aspect-square rounded-[2.5rem] overflow-hidden mb-5 relative shadow-2xl bg-zinc-900 border border-white/5 transition-all group-hover:-translate-y-2">
              ...
              <div className="px-2 space-y-1">
                ...
                <div className="flex items-center gap-2 text-[9px] text-zinc-500 font-black uppercase tracking-wider">
                  <span>{p.songIds.length} Assets</span>
                  <span>•</span>
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <div className="flex items-center gap-3 text-[9px] font-black text-zinc-500 uppercase tracking-[0.2em]">
            <span className="flex items-center gap-1"><FolderOpen size={12} className="text-blue-500" /> Core Library</span>
            <span>{songs.length} Local Files</span>
          </div>
        </div>
      ...
      {isGridView ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
          {songs.map(song => (
            <div 
              ...
              className="group bg-white/[0.01] hover:bg-white/[0.04] border border-white/5 p-4 rounded-[2rem] transition-all cursor-pointer relative"
            >
              ...
              <h4 className={`font-bold text-sm truncate mb-0.5 ${currentSongId === song.id ? 'text-[var(--accent-color)]' : 'text-white'}`}>{song.title}</h4>
              <p className="text-[10px] text-zinc-600 font-bold truncate uppercase">{song.artist}</p>
              <div className="mt-3 flex items-center justify-between border-t border-white/5 pt-2">
                <div className="flex gap-1">
                   {song.isSynced && <CheckCircle size={10} className="text-[var(--accent-color)]" />}
                   {song.hasLyrics && <FileText size={10} className="text-green-500" />}
                </div>
                <span className="text-[8px] text-zinc-700 font-black uppercase">{song.playCount} Tracks</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-1">
          {songs.map((song, idx) => (
            <div 
              key={song.id} onClick={() => onSongSelect(song)}
              className={`group flex items-center gap-4 px-5 py-3 rounded-2xl hover:bg-white/5 transition-all cursor-pointer ${currentSongId === song.id ? 'bg-white/10' : ''}`}
            >
              ...
              <div className="flex-1 flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className={`font-bold text-sm truncate ${currentSongId === song.id ? 'text-[var(--accent-color)]' : 'text-white'}`}>{song.title}</h4>
                  </div>
