...
const HomeView: React.FC<HomeViewProps> = ({ 
  ...
  if (activeView === 'lyrics' && currentSong) {
    return (
      <div className="h-full flex flex-col p-12 animate-in fade-in zoom-in-95 duration-500" dir="rtl">
        <button 
        ...
        >
          <ChevronRight size={14} className="rotate-180" /> بازگشت به داشبورد
        </button>
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar p-10 pb-40 space-y-16" dir="rtl">
      {/* Empty Library State */}
      {library.length === 0 ? (
        <div className="h-full flex flex-col items-center justify-center text-center space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-2xl font-black">کتابخانه شما خالی است</h3>
            <p className="text-zinc-500 max-w-sm">فایل‌های موسیقی خود را به برنامه اضافه کنید تا تجربه Melodix آغاز شود.</p>
          </div>
        ...
        <>
          ...
          <section className="relative h-80 rounded-[3.5rem] overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-l from-[var(--accent-color)]/20 to-transparent z-10" />
            <img 
            ...
            <div className="relative z-30 h-full flex flex-col justify-end p-12 text-right">
              <div className="flex items-center gap-3 mb-4 justify-end">
                <span className="text-zinc-400 text-xs font-bold">بر اساس سلیقه شما</span>
                <span className="px-4 py-1.5 rounded-full bg-[var(--accent-color)] text-white text-[10px] font-black uppercase">پیشنهاد هوشمند</span>
              </div>
              <h1 className="text-6xl font-black text-white tracking-tighter mb-6 leading-[0.9]">موسیقی را هوشمندانه تجربه کنید</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
              <div className="flex items-center justify-between">
                <button className="text-zinc-500 hover:text-[var(--text-primary)] text-xs font-bold transition-all">مشاهده همه</button>
                <h3 className="text-2xl font-black flex items-center gap-3">
                  آخرین پخش شده‌ها <Clock className="text-[var(--accent-color)]" />
                </h3>
              </div>
            ...
            <div className="space-y-8">
              <h3 className="text-2xl font-black flex items-center gap-3 justify-end">
                تحلیل هوشمند <Sparkles className="text-purple-500" />
              </h3>
              <div className="p-8 bg-gradient-to-br from-purple-600/10 to-[var(--accent-color)]/10 border border-white/5 rounded-[2.5rem] space-y-6 relative overflow-hidden group text-right">
                <TrendingUp className="absolute -left-4 -bottom-4 text-white/5 group-hover:scale-110 transition-transform duration-1000" size={120} />
                <div className="space-y-2">
                  <p className="text-xs text-purple-400 font-black uppercase tracking-widest">Mood Analysis</p>
                  <h4 className="text-3xl font-black text-[var(--text-primary)]">وضعیت فعلی: <span className="text-[var(--accent-color)]">{moodAnalysis.mood}</span></h4>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <MotionDiv 
                      ...
                      className="h-full bg-gradient-to-l from-purple-500 to-[var(--accent-color)]" 
                    />
                  </div>
                </div>
              </div>
...
const LyricsContent: React.FC<{ currentSong: Song, lyrics: string, isLoading: boolean, currentTime: number }> = ({ currentSong, lyrics, isLoading, currentTime }) => {
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row-reverse gap-16 overflow-hidden">
      {/* Album Side */}
      <div className="w-full lg:w-1/3 space-y-10 text-right">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto space-y-12 p-12 custom-scrollbar text-center lg:text-right"
        style={{ maskImage: 'linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)' }}
      >
        {isLoading ? (
          <div className="h-full flex flex-col items-center justify-center gap-8">
            <Sparkles className="text-[var(--accent-color)] animate-pulse" size={64} />
            <p className="font-black uppercase tracking-widest text-xs animate-pulse">در حال همگام‌سازی عصبی...</p>
          </div>
        ) : lyricsData.length > 0 ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-zinc-800 opacity-20">
             ...
             <p className="text-2xl font-black italic">متن این قطعه یافت نشد</p>
          </div>
        )}
      </div>
    </div>
