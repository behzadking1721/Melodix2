...
import { cacheItem, getCachedItem } from "./dbService";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

export const fetchLyrics = async (title: string, artist: string, songId: string): Promise<string> => {
  // 1. Check Offline Cache First
  const cached = await getCachedItem('lyrics', songId);
  ...
  try {
    ...
    // 2. Persist to Neural Cache
    if (lyrics.length > 50) await cacheItem('lyrics', songId, lyrics);
    return lyrics;
  } catch (error) {
...
export const suggestSongTags = async (song: Partial<Song>): Promise<Partial<Song>> => {
  const cacheKey = `meta-${song.id}`;
  const cached = await getCachedItem('metadata', cacheKey);
  if (cached) return cached;
  ...
  try {
    ...
    await cacheItem('metadata', cacheKey, data);
    return data;
  } catch (error) {
